...
#setwd("/Users/ameenabu-hanna/AllSWork/WenweiR/WenweiNivel")
...
## AMEEN: command doesn't work for me. Should be:
vars_selected <-read.csv(file = "variable_selections.csv",sep=',')
...
data_first_treatment_relevant_0.4_v0<- dplyr::select(data_first_treatment_relevant_0.4,-one_of(vars_zero_names))
...
dim(data_first_treatment_relevant_0.4_v0)[1] - dim(na.omit(data_first_treatment_relevant_0.4_v0))[1]

data_first_treatment_relevant_0.4_v0_na.omit <- na.omit(data_first_treatment_relevant_0.4_v0)

vars_relevant_0.4_v0_na.omit <- names(data_first_treatment_relevant_0.4_v0_na.omit)
...
# drop variables with variance = 0
data_first_treatment_relevant_0.4_na.omit_v0<- dplyr::select(data_first_treatment_relevant_0.4_v0_na.omit,-one_of(vars_zero_na.omit_names))
...
model_OR <-glm(
  ...
  family = binomial,
  data = data_first_treatment_relevant_0.4_v0 )
...
model_AICo <- glm(
  outcome_4 ~ .,
  data = dplyr::select(data_first_treatment_relevant_0.4_na.omit_v0, -vars_treatment),
  family="binomial")
...
model_AICt <- glm(
  AB_nose_infection ~ .,
  data = select(data_first_treatment_relevant_0.4_na.omit_v0[!names(data_first_treatment_relevant_0.4_na.omit_v0) %in% vars_outcome],-c('type_AB_nose')),
  family=binomial)
...
models <- lapply(vars_AICt,
  function(x) {
    glm(substitute(outcome_4 ~ i, list(i = as.name(x))),
        family = binomial,
        data = select(data_first_treatment_relevant_0.4_na.omit_v0,-vars_treatment))
  }
)
...
data_confounders_to <- data_first_treatment_relevant_0.4_na.omit_v0[c(
...
## Ameen: Does not work for me: object 'outcome_4' not found. I think you are missing the name of the dataset. Also it is very dangerous because glm.obj was defined before!

summary(glm.obj)
## Ameen: this will show the old object glm.obj because the glm command before did not work
...
model_PS <-glm(
  as.formula(paste("AB_nose_infection ~ ",paste(vars_confounders,collapse="+"),sep="")),
  family = "binomial",
  data = data_first_treatment_relevant_0.4_na.omit_v0)

# add propensity score to data set with all covariates, but without missing
data_first_treatment_relevant_0.4_na.omit_v0$pscore <- model_PS$fitted.values

# plot propensity score
labs <- paste("Did patient actually received treatment?:", c("No", "Yes"))
data_first_treatment_relevant_0.4_na.omit_v0 %>%
...
model_Match <- matchit(
  as.formula(paste("AB_nose_infection ~ ",paste(vars_confounders,collapse="+"),sep="")),
  data=data_first_treatment_relevant_0.4_na.omit_v0,replace= TRUE,
  method = "nearest")
#Warning message:
...
## AMEEN: it is better to use a different name than "AB" when you override the value
...
cats <- sapply(data_first_treatment_relevant_0.4_na.omit_v0, is.factor)
...
d<-stddiff.binary(data_first_treatment_relevant_0.4_na.omit_v0[,cats],gcol=1,vcol=c(2:dim(data_first_treatment_relevant_0.4_na.omit_v0[,cats])[2]))

## AMEEN: the warning is because you also included type_AB_nose in the analysis and it was not supposed to be used. But you can just ignore type_AB_nose. All other variables do not have NaNs but numbers.
...
## Ameen: in table d you do not need type_AB_nose.
...
## AMEEN: this happens because you have type_AB_nose as variable in the stddiff analysis. It is not needed. You can ignore the NaN warning because you don't want to analyse type_AB_nose.
...
## AMEEN: did we decide to use postal code?
...
## AMEEN: this does not work. Didn't you mean model_MNPS_cto ? THERE IS NO model_MNPS.
## In order to continue I added the command model_MNPS <- model_MNPS_cto and the added the summary command

## AMEEN ADDED THESE TWO COMMANDS:
model_MNPS <- model_MNPS_cto
summary(model_MNPS)

plot(model_MNPS, plots = 1) 
plot(model_MNPS, plots = 2, subset = "es.mean")

## AMEEN: ADDED THE FOLLOWING COMMAND:
plot(model_MNPS, plots = 2, subset = "ks.mean")

data_first_treatment_relevant_0.4_na.omit_v0$w <- get.weights(data_first_treatment_relevant_0.4_na.omit_v0, stop.method = "es.mean")

## AMEEN: the above does NOT work. data_first_treatment_relevant_0.4_na.omit_v0 is not an mnps or ps object.

## AMEEN: MAYBE YOU MEANT TO WRITE:
data_first_treatment_relevant_0.4_na.omit_v0$w <- get.weights(model_MNPS_cto, stop.method = "es.mean")

design.mnps <- svydesign(ids=~1, weights=~w, data=data_first_treatment_relevant_0.4_na.omit_v0)#

glm1 <- svyglm(outcome_4 ~ as.factor(type_AB_nose), design = design.mnps, family=binomial)

## AMEEN: THERE WAS A WARNING BECAUSE THE NUMBER OF SUCCESSES IS NOT INTEGER. I SUGGEST TO USE: (and you will get the same model)

glm1 <- svyglm(outcome_4 ~ as.factor(type_AB_nose), design = design.mnps, family=quasibinomial)

## AMEEN: BELOW YOU USE model_MNPS_cto again. I used it before.
...
## AMEEN: please check the use of model_MNPS_cto and model_MNPS
...
AB_0 <- data_first_treatment_relevant_0.4_na.omit_v0%>% filter (type_AB_nose == 0) ## control
regF_0 <- rfsrcSyn(
...
pred.Syn_type_0 <- rfsrcSyn(object = regF_0, newdata = data_first_treatment_relevant_0.4_na.omit_v0)

### treatment using subset data with treatment AB 
AB_1 <- data_first_treatment_relevant_0.4_na.omit_v0%>% filter (AB_nose_infection == 1)

regF_1 <- rfsrcSyn(
  as.formula(paste("outcome_4 ~ ", paste(vars_confounders, collapse="+"),sep="")),
  data  = AB_1)
  
pred.Syn_AB_1 <- rfsrcSyn(object = regF_1, newdata = data_first_treatment_relevant_0.4_na.omit_v0)

# calculate difference
data_first_treatment_relevant_0.4_na.omit_v0$delta <- (pred.Syn_AB_1$rfSynPred$predicted -  pred.Syn_type_0$rfSynPred$predicted)[,2]


fit <- glm(
  as.formula(paste("delta ~ ", paste(vars_confounders, collapse="+"),sep="")),
  data  = data_first_treatment_relevant_0.4_na.omit_v0, family = gaussian)

## AMEEN: again, did we decide to use postal code?
...
for (j in 1:5) {
  ...
  AB_j <- data_first_treatment_relevant_0.4_na.omit_v0%>% filter (type_AB_nose == j)
  ...
  pred.Syn_type_j <- rfsrcSyn(object = regF_j, newdata = data_first_treatment_relevant_0.4_na.omit_v0)
  # calculate difference
  data_first_treatment_relevant_0.4_na.omit_v0$delta_j <- (pred.Syn_type_j$rfSynPred$predicted -  pred.Syn_type_0$rfSynPred$predicted)[,2]
  
  
  fit_j <- glm(delta_j ~ age + nr_medication + kanker_morb + coronhartz_morb +  RA_morb + alcmisb_morb + COPD_morb + hartritme_morb + migraine_morb + nr_prescriptions_AB +nr_chron3 + practice_size + chrnek_morb + hartfalen_morb + nr_contacts_infection + nr_contacts_resp + postalcode,  data  = data_first_treatment_relevant_0.4_na.omit_v0, family = gaussian)
...
#  write.csv(table, file=sprintf('fit_delta_%s.csv',j))
  ...
  data_first_treatment_relevant_0.4_na.omit_v0[,ncol(data_first_treatment_relevant_0.4_na.omit_v0)+1]<-data_first_treatment_relevant_0.4_na.omit_v0$delta_j
  names(data_first_treatment_relevant_0.4_na.omit_v0)[ncol(data_first_treatment_relevant_0.4_na.omit_v0)]<-paste0("delta_",j)
}
###########################
...
set.seed(2)
train<-sample(1:nrow(data_first_treatment_relevant_0.4_na.omit_v0),0.8*nrow(data_first_treatment_relevant_0.4_na.omit_v0))
data_first_treatment_relevant_0.4_na.omit_v0.train<-data_first_treatment_relevant_0.4_na.omit_v0[train,]
class(data_first_treatment_relevant_0.4_na.omit_v0.train)

data_first_treatment_relevant_0.4_na.omit_v0.test<-data_first_treatment_relevant_0.4_na.omit_v0[-train,]
outcome_4.test<-data_first_treatment_relevant_0.4_na.omit_v0$outcome_4[-train]  

### Build SRF model in train subset, predic SRF model in test subset

regF <- rfsrcSyn(
  as.formula(paste("outcome_4 ~ ", paste(vars_confounders, collapse="+"),sep="")),
  data  = data_first_treatment_relevant_0.4_na.omit_v0.train)

### using that synthetic forest to predict outcome of test participants (so wether they had treatment of not)
pred.Syn <- rfsrcSyn(object = regF, newdata = data_first_treatment_relevant_0.4_na.omit_v0.test)


pred<- pred.Syn$ rfSynPred$predicted
data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment<- ifelse(data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection == 1,pred[,2], pred[,1])

sum(data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment, na.rm=TRUE)

library("Hmisc")
plsmo(pred_truetreatment,data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection == 1, xlab="predictions", ylab="% events") # This is a smoother and gives the calibration graph. The default of smoothing window is 2/3. Calibration is not bad.

## AMEEN: error: object 'pred_truetreatment' not found. Do you mean pred?

plsmo(pred_truetreatment,data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection == 1, xlab="predictions", ylab="% events", f=1/3) # bumpy

## AMEEN: error: object 'pred_truetreatment' not found

plsmo(pred_truetreatment, data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection == 1, xlab="predictions", ylab="% events", group = data_first_treatment_relevant_0.4_na.omit_v0$age > median(data_first_treatment_relevant_0.4_na.omit_v0$age)) 

## AMEEN: error: object 'pred_truetreatment' not found

# check per group (here above and below median BMI)
# how to define imaginary line vs full line???

range(data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment, na.rm=TRUE) # very good range but let's also look now at distribution of predicitons:
plsmo(pred_truetreatment, data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection == 1, xlab="predictions", ylab="% events", datadensity=T, na.rm=TRUE) # you need to zoom in on the graph to see the tick marks
...
data_first_treatment_relevant_0.4_na.omit_v0.test$OutcomeCstat<-0
data_first_treatment_relevant_0.4_na.omit_v0$treatCstat[as.numeric(data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection)==2]<-1
data_first_treatment_relevant_0.4_na.omit_v0$treatCstat[as.numeric(data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection)==1]<-0

somers2(as.numeric(data_first_treatment_relevant_0.4_na.omit_v0.test$pred_truetreatment),as.numeric(data_first_treatment_relevant_0.4_na.omit_v0.test$treatCstat))

## AMEEN: error: must have at least two non-missing observations

somers2(data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment,data_first_treatment_relevant_0.4_na.omit_v0$treatCstat)

## AMEEN: strange result, the AUC is 100%
  
auc <- somers2(data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment, data_first_treatment_relevant_0.4_na.omit_v0$treatCstat, na.rm = TRUE ) 
(auc <- somers2(data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment, data_first_treatment_relevant_0.4_na.omit_v0$treatCstat )["C"]) # this is the same as somers2(predictions, pima$testC == 1)["C"] but more compact
(brier   <- mean((data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment - data_first_treatment_relevant_0.4_na.omit_v0$treatCstat)^2))

## AMEEN: AGAIN the AUC = 1, this is strange


library("rms")
# Let us get unbiased estimates of the performance
dd <- datadist(data_first_treatment_relevant_0.4_na.omit_v0)  ###how to explain???
options(datadist='dd')

model <- lrm( as.formula(paste("outcome_4 ~ ", paste(vars_confounders, collapse="+"),sep="")), x=T, y=T, data=data_first_treatment_relevant_0.4_na.omit_v0)
v <- validate(model, method="boot", B=100, pr=F)

## There was a warning because datadist had extra levels for postalcode. But I think it is not  a problem.

v # look at table of (bias corrected) performance measure
v[1,5] # This is the adjusted Dxy
v[1,5]/2 + 0.5 # and this is the AUC

#why the AUC is different in the two methods?
# The rms code with the AUC is the correct one  
...
PRIM <- subset(data_first_treatment_relevant_0.4_na.omit_v0, select = c(age, nr_medication, kanker_morb, coronhartz_morb,  RA_morb , alcmisb_morb ,
...
## AMEEN: On the small dataset I get an error for (i in 1:10) when i = 3 and more. Maybe on the big dataset it is OK. But in anycase even if there is an error the result will be saved in prim.table. I added an extra command to print prim.table at the end.
...
## AMEEN: I added the following line:
prim.table
