...
weightmodel<-"ipwpoint(exposure=AB_nose_infection, data = data_confounders_to, family = "binomial", link = "logit",
                   denominator= as.formula(paste(" ~  ", paste(vars_confounders,collapse="+"),sep="")))"
...
executeme<-"
weightmodel<-ipwpoint(exposure = AB_nose_infection, family = binomial"", link = ""logit"", data = data_confounders_to,
                      denominator = ~ age + sex + nr_medication + COPD_morb + migraine_morb + 
hartfalen_morb + nr_contacts_resp + osteop_morb + poor_immune_response + 
alcmisb_morb + postalcode )"
...
model_MNPS_cto <- mnps(
  ...
  stop.method = c("es.mean", "ks.mean"),
  n.trees = 3000)
#Error in mnps(as.formula(paste("type_AB_nose ~ ", paste(vars_confounders,  : 
 #                                                         The treatment variable must be a factor variable with at least 3 levels.
#Warning messages:1: In ps(formula = currFormula, data = currDat, n.trees = n.trees[i],  :
...
data_first_treatment_relevant_0.4_na.omit_v0$pred_truetreatment<- ifelse(data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection == 1,pred[1], pred[0])

sum(pred_truetreatment, na.rm=TRUE)
...
plsmo(pred_truetreatment, data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection == 1, xlab="predictions", ylab="% events", group = data_first_treatment_relevant_0.4_na.omit_v0$age > median(data_first_treatment_relevant_0.4_na.omit_v0$age)) # check per group (here above and below median BMI)
...
somers2(as.numeric(pred[,2]),as.numeric(data_first_treatment_relevant_0.4_na.omit_v0.test$treatCstat))

  
auc <- somers2(pred_truetreatment, data_first_treatment_relevant_0.4_na.omit_v0$treatCstat, na.rm = TRUE )  
(auc <- somers2(pred_truetreatment, data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection )["C"]) # this is the same as somers2(predictions, pima$testC == 1)["C"] but more compact
brier   <- mean((pred_t1-data_first_treatment_relevant_0.4_na.omit_v0$AB_nose_infection)^2)
